...
class SentimentClassifier:
    ...
    def classify_text_sentiment(self, topic, input_text: str) -> str:
        """
    ...
    def run_text_classifier(self,
        ...
        sample_articles = df.sample(10)

        # new col name
        new_col = self.model_name + '_classification'
        # apply the model on the sample articles and store in a new column.
        sample_articles[new_col] = sample_articles[text_col].apply(lambda x:
                                                                   self.classify_text_sentiment(text_col, x)
                                                                   )
