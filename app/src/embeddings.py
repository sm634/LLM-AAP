...
example_article = articles[-1]

model = SentenceTransformer('sentence-transformers/all-MiniLM-L6-v2')
article_vector = model.encode(example_article, convert_to_tensor=True)
...
for indicator in indicators:
    ...
    sim_scores[indicator] = util.pytorch_cos_sim(a=article_vector, b=indicator_vector)

ordered_sim_scores = OrderedDict(sorted(sim_scores.items(),
                                        key=lambda item: item[1],
                                        reverse=True))
print(ordered_sim_scores)
breakpoint()
